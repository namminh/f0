#!/usr/bin/env python3
"""
Create Comprehensive Financial Charts for VHM
Tạo biểu đồ tài chính toàn diện cho VHM
"""
...
def create_financial_charts():
    ...
    # Create directories
    Path("stock_analysis/VHM/charts/financial_analysis").mkdir(parents=True, exist_ok=True)

    # Load data
    try:
        with open("stock_analysis/VHM/data/VHM_financial_data.json", "r", encoding="utf-8") as f:
            financial_data = json.load(f)
    except FileNotFoundError:
        print("Không tìm thấy dữ liệu financial VHM")
        return
    ...
    print("Financial charts created successfully for VHM")

def create_profitability_analysis(financial_data):
    """Phân tích khả năng sinh lời"""
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    
    # Simulated VHM financial data (real estate company)
    ...
    ax1.set_title('VHM - Return on Equity (ROE)', fontweight='bold')
    ax1.set_ylabel('ROE (%)')
    ...
    ax2.set_title('VHM - Return on Assets (ROA)', fontweight='bold')
    ax2.set_ylabel('ROA (%)')
    ...
    ax3.set_title('VHM - Net Profit Margin', fontweight='bold')
    ...
    ax4.set_title('VHM - Profitability Comparison', fontweight='bold')
    ax4.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VHM/charts/financial_analysis/profitability_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_financial_health_dashboard(financial_data):
    ...
    title_text = """
VHM - VINHOMES FINANCIAL HEALTH DASHBOARD
═══════════════════════════════════════════════════════════════════════════════════════

🏢 VINHOMES JOINT STOCK COMPANY - Leading Real Estate Developer
📊 Comprehensive Financial Analysis | 📅 Generated: """ + datetime.now().strftime('%d/%m/%Y %H:%M') + """

Key Real Estate Metrics:
• Revenue Growth: Strong project delivery performance
• Asset Quality: High-value property portfolio  
...
✓ Industry leader
    ...
    ax5.set_title('VHM Risk Assessment', fontweight='bold')
    ...
    ax6.set_title('Real Estate Performance', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VHM/charts/financial_analysis/financial_health_dashboard.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_real_estate_specific_metrics(financial_data):
    ...
    # Revenue by segment
    years = [2020, 2021, 2022, 2023, 2024]
    residential_revenue = [45.2, 52.8, 68.3, 59.7, 74.1]  # Billion VND
    ...
    ax1.set_title('VHM - Revenue by Segment', fontweight='bold')
    ...
    ax2.set_title('VHM - Project Delivery Performance', fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ...
    ax3.set_title('VHM - Land Bank & Inventory', fontweight='bold')
    ax3.grid(True, alpha=0.3)
    ...
    ax4.set_title('VHM - Financial Leverage', fontweight='bold')
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig("stock_analysis/VHM/charts/financial_analysis/real_estate_specific_metrics.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_peer_comparison_template(financial_data):
    ...
    # Real estate company comparison data
    companies = ['VHM', 'VIC', 'NVL', 'VRE', 'DXG']
    ...
    # ROE Comparison
    colors = ['red' if company == 'VHM' else 'lightblue' for company in companies]
    bars1 = ax1.bar(companies, roe_values, color=colors, alpha=0.8)
    ax1.set_title('ROE Comparison - Real Estate Companies', fontweight='bold')
    ax1.set_ylabel('ROE (%)')
    ax1.grid(True, alpha=0.3)
    ...
    for i, (bar, value) in enumerate(zip(bars1, roe_values)):
        ...
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.3,
                f'{value:.1f}%', ha='center', va='bottom', 
                fontweight='bold' if companies[i] == 'VHM' else 'normal')
    
    # ROA Comparison
    colors = ['red' if company == 'VHM' else 'lightgreen' for company in companies]
    bars2 = ax2.bar(companies, roa_values, color=colors, alpha=0.8)
    ax2.set_title('ROA Comparison - Real Estate Companies', fontweight='bold')
    ax2.set_ylabel('ROA (%)')
    ax2.grid(True, alpha=0.3)
    ...
    # VHM metrics (normalized to 0-1 scale)
    vhm_metrics = [
    ...
    vhm_metrics += vhm_metrics[:1]
    
    ax3 = plt.subplot(2, 2, 3, projection='polar')
    ax3.plot(angles, vhm_metrics, 'o-', linewidth=2, label='VHM', color='red')
    ax3.fill(angles, vhm_metrics, alpha=0.25, color='red')
    ax3.set_xticks(angles[:-1])
    ax3.set_xticklabels(['ROE', 'ROA', 'Profit Margin', 'Revenue Growth'])
    ax3.set_ylim(0, 1)
    ax3.set_title('VHM Performance Radar', fontweight='bold', pad=20)
    ax3.grid(True)
    
    # Market position ranking
    metrics = ['ROE', 'ROA', 'Profit Margin', 'Revenue Growth']
    vhm_ranks = [1, 1, 1, 1]  # VHM's ranking (1-5, 1 is best)
    
    ax4.barh(metrics, [6-rank for rank in vhm_ranks], color='gold', alpha=0.7)
    ax4.set_xlim(0, 5)
    ax4.set_xlabel('Ranking (5=Best, 1=Worst)')
    ax4.set_title('VHM Market Position Ranking', fontweight='bold')
    ax4.grid(True, alpha=0.3)
    
    # Add ranking labels
    for i, rank in enumerate(vhm_ranks):
        ax4.text(6-rank + 0.1, i, f'#{rank}', va='center', fontweight='bold')
    
    plt.tight_layout()
    plt.savefig("stock_analysis/VHM/charts/financial_analysis/peer_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_financial_trends_analysis(financial_data):
    ...
    ax1.set_title('VHM - Revenue & Profit Trends', fontweight='bold')
    ...
    land_inventory = [125, 145, 168, 152, 185]  # Trillion VND
    
    ax2.bar(years, total_assets, alpha=0.6, label='Total Assets', color='lightblue')
    ax2.bar(years, land_inventory, alpha=0.8, label='Land & Inventory', color='darkblue')
    ax2.set_title('VHM - Asset Growth', fontweight='bold')
    ...
    ax3.set_title('VHM - Efficiency Trends', fontweight='bold')
    ...
    ax4.set_title('VHM - Leverage & Liquidity Trends', fontweight='bold')
    ax4.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VHM/charts/financial_analysis/financial_trends.png", dpi=300, bbox_inches='tight')
    plt.close()
