...
def create_historical_charts():
    """Tạo biểu đồ lịch sử giá cho VIC"""
    
    # Create directories
    Path("stock_analysis/VIC/charts/historical_analysis").mkdir(parents=True, exist_ok=True)

    # Try to load historical data - check multiple possible locations
    historical_data = None
    possible_files = [
        "stock_analysis/VIC/data/VIC_historical_prices.json",
        "stock_analysis/VIC/data/VIC_historical_3years.json",
        "stock_analysis/VIC/data/VIC_intraday_data.json"
    ]
    ...
    if not historical_data:
        print("Không tìm thấy dữ liệu lịch sử VIC")
        return
    ...
    column_mapping = {
        ...
        'close': 'Close',
        'volume': 'Volume',
        'price': 'Close'  # For intraday data that uses 'price' instead of 'close'
    }
    ...
    if 'Close' not in df.columns:
        if 'price' in df.columns:
            ...
            print("Không tìm thấy cột giá trong dữ liệu")
            return
    ...
    print(f"Creating historical charts for VIC with {len(df)} data points")
    ...
    print("Historical charts created successfully for VIC")
...
def create_price_trend_analysis(df):
    ...
    ax1.set_title('VIC - Xu hướng Giá Lịch sử', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('VIC - Thay đổi Giá Hàng ngày (%)', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('VIC - Phân phối Giá', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('VIC - Tỷ suất Sinh lời Tích lũy', fontsize=14, fontweight='bold')
    ax4.set_ylabel('Tỷ suất tích lũy')
    ax4.grid(True, alpha=0.3)
    ax4.tick_params(axis='x', rotation=45)
    
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/historical_analysis/price_trend_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_volume_trend_analysis(df):
    ...
    ax1.set_title('VIC - Xu hướng Khối lượng Lịch sử', fontsize=14, fontweight='bold')
    ...
    if df['Volume'].sum() > 0:  # Only if we have volume data
    ...
    ax2.set_title('VIC - Tương quan Khối lượng vs Giá', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('VIC - Khối lượng Giao dịch Hàng tháng', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('VIC - Phân phối Khối lượng', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/historical_analysis/volume_trend_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_technical_analysis_historical(df):
    ...
    ax1.plot(df.index, df['Close'], label='Giá VIC', linewidth=2, color='blue')
    ...
    ax1.set_title('VIC - Giá với Bollinger Bands', fontsize=14, fontweight='bold')
    ax1.set_ylabel('Giá (VND)')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.tick_params(axis='x', rotation=45)
    ...
    ax2.set_title('VIC - RSI (Relative Strength Index)', fontsize=14, fontweight='bold')
    ax2.set_ylabel('RSI')
    ax2.set_ylim(0, 100)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    ...
    ax3.set_title('VIC - MACD', fontsize=14, fontweight='bold')
    ax3.set_ylabel('MACD')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ax3.tick_params(axis='x', rotation=45)
    ...
    ax4.plot(df.index, df['Close'], linewidth=2, color='blue', label='Giá VIC')
    ...
    ax4.set_title('VIC - Support & Resistance Levels', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/historical_analysis/technical_analysis_historical.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_performance_analysis(df):
    ...
    ax1.set_title('VIC - Tỷ suất Sinh lời Hàng tháng', fontsize=14, fontweight='bold')
    ...
    ax2.axhline(y=0, color='black', linestyle='-', alpha=0.8)
    ax2.set_title('VIC - Hiệu suất Hàng năm', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('VIC - Risk-Return Profile (Quarterly)', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('VIC - Drawdown Analysis', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/historical_analysis/performance_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_volatility_analysis(df):
    ...
    ax1.set_title('VIC - Rolling Volatility', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('VIC - Daily Returns Distribution', fontsize=14, fontweight='bold')
    ...
    if len(valid_data) > 1 and valid_data['Volume'].sum() > 0:
    ...
    ax3.set_title('VIC - Volatility vs Volume', fontsize=14, fontweight='bold')
    ...
    ax4.set_title('VIC - Volatility Regimes', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/historical_analysis/volatility_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
