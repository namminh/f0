#!/usr/bin/env python3
"""
Create Comprehensive Financial Charts for VIC
Tạo biểu đồ tài chính toàn diện cho VIC
"""
...
def create_financial_charts():
    ...
    # Create directories
    Path("stock_analysis/VIC/charts/financial_analysis").mkdir(parents=True, exist_ok=True)

    # Load data
    financial_data = {}
    try:
        with open("stock_analysis/VIC/data/VIC_balance_sheet.json", "r", encoding="utf-8") as f:
            financial_data['balance_sheet'] = json.load(f)
        with open("stock_analysis/VIC/data/VIC_income_statement.json", "r", encoding="utf-8") as f:
            financial_data['income_statement'] = json.load(f)
        with open("stock_analysis/VIC/data/VIC_financial_ratios.json", "r", encoding="utf-8") as f:
            financial_data['financial_ratios'] = json.load(f)
    except FileNotFoundError:
        print("Không tìm thấy dữ liệu financial VIC")
        return
    ...
    print("Financial charts created successfully for VIC")

def create_profitability_analysis(financial_data):
    """Phân tích khả năng sinh lời"""
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    
    # Simulated VIC financial data (conglomerate)
    ...
    ax1.set_title('VIC - Return on Equity (ROE)', fontweight='bold')
    ax1.set_ylabel('ROE (%)')
    ...
    ax2.set_title('VIC - Return on Assets (ROA)', fontweight='bold')
    ax2.set_ylabel('ROA (%)')
    ...
    ax3.set_title('VIC - Net Profit Margin', fontweight='bold')
    ...
    ax4.set_title('VIC - Profitability Comparison', fontweight='bold')
    ax4.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/financial_analysis/profitability_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_financial_health_dashboard(financial_data):
    ...
    title_text = """
VIC - VINGROUP FINANCIAL HEALTH DASHBOARD
═══════════════════════════════════════════════════════════════════════════════════════
...
✓ Industry leader
    ...
    ax5.set_title('VIC Risk Assessment', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/financial_analysis/financial_health_dashboard.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_real_estate_specific_metrics(financial_data):
    ...
    ax1.set_title('VIC - Revenue by Segment', fontweight='bold')
    ...
    ax2.set_title('VIC - Project Delivery Performance', fontweight='bold')
    ax2.grid(True, alpha=0.3)
    ...
    ax3.set_title('VIC - Land Bank & Inventory', fontweight='bold')
    ax3.grid(True, alpha=0.3)
    ...
    ax4.set_title('VIC - Financial Leverage', fontweight='bold')
    ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/financial_analysis/real_estate_specific_metrics.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_peer_comparison_template(financial_data):
    ...
    # ROE Comparison
    colors = ['red' if company == 'VIC' else 'lightblue' for company in companies]
    ...
    for i, (bar, value) in enumerate(zip(bars1, roe_values)):
        ...
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.3,
                f'{value:.1f}%', ha='center', va='bottom', 
                fontweight='bold' if companies[i] == 'VIC' else 'normal')
    
    # ROA Comparison
    colors = ['red' if company == 'VIC' else 'lightgreen' for company in companies]
    ...
    # VIC metrics (normalized to 0-1 scale)
    vic_metrics = [
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/financial_analysis/peer_comparison.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_financial_trends_analysis(financial_data):
    ...
    ax1.set_title('VIC - Revenue & Profit Trends', fontweight='bold')
    ...
    ax2.set_title('VIC - Asset Growth', fontweight='bold')
    ...
    ax3.set_title('VIC - Efficiency Trends', fontweight='bold')
    ...
    ax4.set_title('VIC - Leverage & Liquidity Trends', fontweight='bold')
    ax4.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig("stock_analysis/VIC/charts/financial_analysis/financial_trends.png", dpi=300, bbox_inches='tight')
    plt.close()
